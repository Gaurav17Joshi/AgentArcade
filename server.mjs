...
function runAnthropicKlotskiAgent({level,model,onEvent,task,solverMode}){const env=parseKlotski(level);return runAnthropicStructuredAgent({kind:'Klotski',env,serialise:serialiseKlotski,apply:applyKlotski,solve:solveKlotski,isSolved:state=>{const target=state.pieces.find(piece=>piece.target);return Boolean(target&&target.y===state.exitRow&&target.x===state.cols-target.w)},model,onEvent,task,solverMode});}
